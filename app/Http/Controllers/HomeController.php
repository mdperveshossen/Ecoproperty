...
class HomeController extends Controller
{
    ...
    function changephoto(Request $request)
    {
        if ($request->hasFile('profile_photo')) {
            if (Auth::user()->profile_photo != 'default.jpg') {
                $old_photo_location = 'uploads/profile_photos/' . Auth::user()->profile_photo;
                unlink(base_path($old_photo_location));
            }
            $uploaded_photo = $request->file('profile_photo');
            $new_file_name = Auth::id() . "." . $uploaded_photo->getClientOriginalExtension();
            $new_file_location = 'uploads/profile_photos/' . $new_file_name;
    ...
    public function homelogoupdatepost(Request $request, $id){
        if ($request->hasFile('logo')) {
            if (Logo::find($id)->logo != 'default.png') {
                $old_photo_location = 'uploads/logo/' . Logo::find($id)->logo;
                unlink(base_path($old_photo_location));
            }
    ...
    public function navbar(){
        return view('admin.navbar.index', [
            'items' => Navbar::latest()->get(),
        ]);
    }
    public function navbarupdate($id){
        return view('admin.navbar.update_navbar', [
            'info' => Navbar::find($id)
        ]);
    }
    public function navbarupdatepost(Request $request, $id){
        Navbar::find($id)->update($request->except('_token'));
        return back()->with('update_nav', 'Nav item updated successfully!!');
    }
}
